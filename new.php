...
$d_encounter_id = (isset($fs->encounter) && $fs->encounter) ? $fs->encounter : '';
?>
<html>
...
//     array($pid, $encounter)
// );
function getEncounterCatData($encounter,$field=null) {
  $row = sqlQuery("SELECT id, pc_catid FROM form_encounter WHERE encounter = ?", array($encounter));
  if ($row) {
    $pc_catid = $row['pc_catid'];
    $category = sqlQuery("SELECT pc_catid, pc_catname FROM openemr_postcalendar_categories WHERE pc_catid = ?", array($pc_catid));
    if($category) {
      if($field) {
        $data = ( isset($category[$field]) && $category[$field] ) ? $category[$field] : '';
        return $data;
      } else {
        return $category;
      }
...
$enrow = sqlQuery("SELECT p.fname, p.mname, p.lname, fe.date FROM patient_data p, form_encounter fe  WHERE fe.pid=p.pid AND p.pid = ? AND fe.encounter = ?", array($pid, $encounter));
...
// $heading =  join(" ", array_filter($title));
$encCatName = getEncounterCatData($encounter,'pc_catname');
$heading = 'Fee Sheet for ';
$heading .= $name;
$heading .= ' ('.$encCatName . ')';
?>
<?php
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="page-header clearfix" style="margin-top:15px;">
                    ...
                    <h2 style="margin: 0 0 10px">Fee Sheet for <?php echo $name ?></h2>
                    <h3 style="margin: 0 0;font-size:16px;font-weight:normal;line-height:1.3;">
                      Note: <?php echo $encCatName ?><BR>
                      DOS: <?php echo oeFormatShortDate(substr($enrow['date'], 0, 10)) ?>
                    </h3>
                </div>
            </div>
       </div>
                        ...
                        <div class="col-sm-12 position-override">
                            <div class="btn-group oe-opt-btn-group-pinch" role="group">
                                ...
                                <?php if (!$isBilled) { // visit is not yet billed ?>
                                    ...
                                    } ?>><?php echo xla('Save and Close');?></button>
