...
/* SAVE NEW COLUMNS */
function saveCalendarFieldAddons($args,$eid) {
    $fields[] = array(
      'field'=>'status',
      'value'=>(isset($args['feestatus']) && $args['feestatus']) ? $args['feestatus'] : null
    );
    $fields[] = array(
      'field'=>'comments',
      'value'=>(isset($args['form_other_comments']) && $args['form_other_comments']) ? nl2br($args['form_other_comments']) : null
    );

    foreach($fields as $col) {
      $fieldName = $col['field'];
      $fieldVal = $col['value'];
      sqlStatement("UPDATE openemr_postcalendar_events_additional SET ".$fieldName." = ? WHERE pc_eid = ?", array($fieldVal,$eid));
    }
}

/* GET NEW COLUMN DATA */
function getCalendarFieldData($eid,$field) {
  $addtlData = sqlQuery("SELECT * FROM openemr_postcalendar_events_additional WHERE pc_eid = ?", array($eid));
  return (isset($addtlData[$field]) && $addtlData[$field]) ? $addtlData[$field] : '';
}
...
if ($_POST['form_action'] == "save") {
    ...
    //if($_POST['provider_tab']){
        ...
        //$endtime = "$form_hour_end:$form_minute_end:00";
        $endtime_h = (($form_ampm_end == 2) && ($form_hour_end < 12)) ? $form_hour_end + 12 : $form_hour_end;
        $endtime_m = $form_minute_end;
        $endtime_ampm = ($form_ampm_end == 1) ? 'AM' : 'PM';
    ...
    if ($eid) {
        ...
        // oct-08 JRM
            ...
            $status_sym_cancelled = array('?', '%');
            $status = $_POST['form_apptstatus'];

            if( !empty($status) && in_array($status, $status_sym_cancelled) ){
                // trigger the email here

                $pid                = $_POST['form_pid'];
                $catid              = $_POST['form_title'];
                $name               = $_POST['form_patient'];
                ...
                if($pid && $catid && $name){
                    $msg_status = send_cancelled_status($data, $status);
                }
        ...
        // $otherStatus = (isset($args['feestatus']) && $args['feestatus']) ? $args['feestatus'] : null;
        // sqlStatement("UPDATE openemr_postcalendar_events_additional SET status = ? WHERE pc_eid = ?", array($otherStatus,$eid));

        /* ADD / UPDATE NEW COLUMNS */
        saveCalendarFieldAddons($_POST,$eid);
...
function set_display() {
    ...
    <?php if($_GET['prov'] == false && $_GET['group'] == false): ?>
        tabActive = true;
    <?php endif; ?>
    if(f.form_title.value == 'Select' && tabActive){
        f.form_title.value = 'Non-Billable Contact';
    }
...
<style>
  #theform label{position:relative;}
  #theform .button-group.buttonbar {
    margin-top: 10px;
    margin-bottom: 10px;
  }
  #theform .button-group.buttonbar input[type="button"]{
    padding: 3px 15px;
    border-radius: 4px;
    margin-right: 3px;
    transition: all ease .2s;
  }
  #theform .button-group.buttonbar input[type="button"]:hover {
    background: #1cbd60;
  }
  #theform .well.well-sm label:not(.w-auto),
  #theform .form-group label:not(.w-auto) {
    display: inline-block;
    width: 130px;
    padding-right: 0!important;
  }
  #theform .well.well-sm .input-group label {
    width: auto;
  }
  #theform label[for="new_patient"] {
    left: -12px;
  }
  #theform label[for="new_patient"] input {
    top: -2px;
  }
  #theform label[for="billing_facility"] {
    left: 6px;
  }
  #theform .form-control {
    border-radius: 0!important;
  }
  #theform select[name="form_provider"],
  #theform select#facility {
    width: 205px;
  } 
  #theform .form-control[name="form_patient"] {
  ...
  #theform input[name="new_patient"],
  #theform .patient_type_row input[type="radio"]{
    box-shadow: none!important;
  }
  #theform .form-control[name="form_title"],
  #theform .form-control[name="form_comments"],
  #theform .form-control#form_category,
  #theform textarea[name="form_other_comments"] {
  ...
  #theform .flexbox-fields select#feestatus,
  #theform select#form_apptstatus {
    width: 180px!important;
  }
  #theform label.allDayLabel {
    padding-right: 0!important;
    padding-left: 0!important;
  }
  #theform input[type="radio"][name='form_allday'] {
    position: relative;
    top: 2px;
  }
  #theform .form-control[name="form_date"] {
    width: 105px;
  }
  #theform input[type="checkbox"][name='form_repeat'] {
    position: relative;
    top: 2px;
  }
  #theform label[for="form_other_comments"] {
    top: -37px;
    line-height: 1.2;
    font-size: 14px!important;
  }
  #theform #days_every_week_row #days {
    position: relative;
    left: 4px;
  }
  #theform #days_every_week_row #days label {
    display: inline-block;
    position: relative;
    padding: 0 0 0 10px!important;
    width: 50px;
  }
  #theform #days_every_week_row #days label input {
    margin: 0 0;
    position: absolute;
    top: 4px;
    left: -13px;
  }
  #theform label[for="type_patient_medicaid"],
  #theform label[for="type_patient_private"] {

  }
  #theform label[for="type_patient_medicaid"] input,
  #theform label[for="type_patient_private"] input {
    position: relative;
    left: 2px;
    top: -2px;
  }
  #theform span.form_hour_end_label {
    position: relative;
    left: 24px;
  }
  .navbar.navbarPopup {
  ...
  .navbar.navbarPopup ul.tabNav {
    border-bottom: 1px solid #2ecc71;
  }
  .navbar.navbarPopup ul.tabNav li {
    margin-right: 3px!important;
    position: relative;
    top: 1px;
  }
  .navbar.navbarPopup ul.tabNav li.current {
    top: -1px;
  }
  .navbar.navbarPopup ul.tabNav li.current a {
    border-top-width: 4px!important;
  }
  .navbar.navbarPopup ul.tabNav li.current a:hover {
    text-decoration: none!important;
  }
  .navbar.navbarPopup ul.tabNav li:not(.current) a {
    border-top: 4px solid #c0efd4!important;
    border-left: 1px solid #c0efd4!important;
    border-right: 1px solid #c0efd4!important;
    padding: 4px 10px 5px;
  }
  .navbar.navbarPopup ul.tabNav li:not(.current) a:hover {
  ...
  .modal-content div.closeDlgIframe {
    border-radius: 0!important;
    top: 4px!important;
    right: 4px!important;
  }
...
<div class="row patient_type_row">
    <div class="radio" style="display: inline-block; margin-right: 40px">
        <span style="margin-right: 20px"><strong style="position:relative;left: 3px;"><?php echo xlt('Type of Patient:'); ?></strong></span>
        <label for="type_patient_medicaid" class="radio-inline">
            ...
            <?php echo is_event_has_form($eid) ? " disabled ": "" ?>
    ...
    <div class="checkbox" style="display: inline-block; ">
        ...
        <label for="new_patient">
            <input type="checkbox" name="new_patient" id="new_patient" value="1" class="form-control" <?php echo ($row['new_patient'] == 1) ? " checked ": ""; ?> <?php echo is_event_has_form($eid) ? " disabled ": "" ?>> <?php echo xlt('New Patient'); ?>
        </label>
    </div>
...
<div class="row">
    <div class="form-group">
        ...
        <select class='form-control' name='form_category' id='form_category' onchange='set_category()' <?php echo is_event_has_form($eid) ? " readonly ": "" ?>>
            <option value="">Select</option>
        </select>
    </div>
    <div class="form-group">
        <label><?php echo xlt('Title'); ?>:</label>
        <input class="form-control" type='text'  name='form_title' value='<?php echo attr($row['pc_title']); ?>'  <?php echo is_event_has_form($eid) ? " readonly ": "" ?>
...
<div class="row">
    ...
    <?php  
    /* OTHER COMMENTS */
    $otherComments = (isset($eid) && $eid) ? getCalendarFieldData($eid,'comments') : '';
    ?>
    <div class="form-group">
        <label for="form_other_comments"><?php echo xlt('Additional Issues'); ?>:</label>
        <textarea class='form-control-x' name="form_other_comments" id="form_other_comments"><?php echo $otherComments; ?></textarea>
    </div>
</div>
<div class="row">
    <div class="form-group">
        <label for="facility"><?php echo xlt('Location'); ?>:</label>
    ...
    <div class="form-group">
    <label for="billing_facility" class="w-auto"><?php echo xlt('Billing Location'); ?>:</label>
        <?php
...
if ($_GET['prov'] != true && $_GET['group'] != true) { ?>
    <div class="row well well-sm" id="patient_details">
        <div class="form-group ">
            ...
            <input type="hidden" name="has_message" id="has_message" value="">
        </div>
        <!-- Patient phone numbers -->
        <div class="form-group" style='font-size:11pt'>
            <?php if($patient_phone):  ?>
                <span class="infobox text-success" style="position:relative;left:8px;">
...
<div class="row">
    <div class="row">
        <div class="form-group">
            ...
            <div class="form-group">
                <label class="w-auto allDayLabel"><?php echo xlt('Date'); ?>:</label>
...
<div class="row">
  <div class="flexbox-fields">
    <div class="form-group flexfield">
    ...
    $feeStatusOptions = array();
    $status_label = '';
    if(file_exists(__DIR__ . '/calendar_options.json')) {
      $nJson = file_get_contents('./calendar_options.json');
      $newOpt = ($nJson) ? json_decode($nJson) : '';
      $feeStatusOptions = ( isset($newOpt->status) && $newOpt->status ) ? $newOpt->status : '';
      $status_label = ( isset($newOpt->status_label) && $newOpt->status_label ) ? $newOpt->status_label : '';
    }

    /* SMILEY ICON */
    $smileyStatus = (isset($eid) && $eid) ? getCalendarFieldData($eid,'status') : '';
    if($feeStatusOptions) { ?>
    <div class="form-group flexfield">
      <label id="title_feestatus" class="w-auto"><?php echo ($status_label) ? xlt($status_label) : 'Unknown'; ?>:</label>
      <select class='form-control input-sm' name="feestatus" id="feestatus">
        <option value="">Select...</option>
        <?php foreach ($feeStatusOptions as $opt) {
          $is_selected = ($smileyStatus && ($smileyStatus==$opt->slug) ) ? ' selected':''; ?>
          <option data-icon="<?php echo $opt->icon ?>" value="<?php echo $opt->slug ?>"<?php echo $is_selected?>><?php echo $opt->name ?></option>
        <?php } ?>
      </select>
    </div>
    <?php } ?>
...
function patient_notes_expired(form_pid, form_category){

    console.log('form_category: ' + form_category);

    $.ajax({
        method: 'POST',
        url: "<?php echo $web_root; ?>/interface/main/calendar/get_tx_status.php",
        data: {
            form_pid: form_pid,
...
function checkNotesValue(value, arr) {
    var status = false;
...
function validateform(event,valu){
    ...
    <?php if (!$GLOBALS['allow_early_check_in']) { ?>
        ...
        var appt_status = $('#form_apptstatus').val();
        var appt_status_label = '';

        if(appt_status == '@'){
            appt_status_label = 'Arrive';
        }
        if(appt_status == '~'){
            appt_status_label = 'Arrive Late';
        }
        if(appt_status == '>'){
            appt_status_label = 'Session Completed';
        }

        if( ( (appt_status == '@') || (appt_status == '~') || (appt_status == '>') ) && new Date(DateToYYYYMMDD_js($('#form_date').val())).getTime() > new Date().getTime()){
            var app_message = "You can not change status to '" + appt_status_label +"' before the appointment's time.";
            alert(app_message);
            $('#form_save').attr('disabled', false);
            return false;
        }
    ...
    <?php if( !( ($_GET['prov'] == true) || ($_GET['group'] == true) ) ): ?>
        get_payment_message();
        var have_message = $('#has_message').val();
        //console.log(have_message);
        if(have_message !== ''){
            alert(have_message);
            $('#form_save').attr('disabled', false);
            return false;
...
function get_payment_message(){
    ...
    var csrf_token_form = "<?php echo attr(CsrfUtils::collectCsrfToken()); ?>";
