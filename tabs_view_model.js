 ...
 *
 * @package   OpenEMR
 * @link      https://mher40.wixsite.com/website
...
function activateTabByName(name,hideOthers)
{
    ...
    for(var tabIdx=0;tabIdx<app_view_model.application_data.tabs.tabsList().length;tabIdx++)
    {
        ...
        if(curTab.name()===name)
        {
            ...
            parentNode.classList.add("iframe-display-"+name);
        }
        else if(hideOthers)
        {
            if(!curTab.locked())
...
function menuActionClick(data,evt)
{
    ...
    if(data.enabled())
    {
        ...
        var dataLabel = data.label().trim();
        var matches = dataurl.match(/load_form.php\?formname=(\w+)/);
        if (matches) {

            /* Don't run this code if it's Fee Sheet form  */
            if( dataLabel!=='Fee Sheet' ) {
          ...
          dataurl = '/interface/patient_file/encounter/encounter_top.php?formname=' + matches[1] + '&formdesc=' + encodeURIComponent(data.label());
        }

        navigateTab(webroot_url + dataurl, data.target, function () {
          var target = data.target;
          activateTabByName(target,true);
        });
